...
def worker_task(args):
    weighted_numbers, num_to_play = args
    return generate_combination(weighted_numbers, num_to_play)
...
def montecarlo_lottery_simulation(numbers, frequencies, num_to_play, iterations, processes):
    ...
    with Pool(processes) as pool:
        combinations = list(tqdm(pool.imap(worker_task, [(weighted_numbers, num_to_play)] * iterations), total=iterations, desc="Simulando"))
